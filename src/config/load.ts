/**
 * Config loading: file config over defaults, then env overrides.
 *
 * Env: DEEPSEEK_API_KEY, AIRICLAW_MODEL, AIRICLAW_BASE_URL,
 * AIRICLAW_THINKING_MODE, AIRICLAW_GATEWAY_PORT, AIRICLAW_SKILLS_DIR,
 * AIRICLAW_MEMORY_DIR.
 */
...
const THINKING_MODES: readonly ThinkingMode[] = ['non_think', 'think_high', 'think_max']
...
export async function loadConfig(cwd: string = process.cwd()): Promise<AppConfig> {
  ...
  if (
    env.AIRICLAW_THINKING_MODE
    && THINKING_MODES.includes(env.AIRICLAW_THINKING_MODE as ThinkingMode)
  ) {
  ...
  if (env.AIRICLAW_GATEWAY_PORT) {
    ...
    if (Number.isFinite(port) && port > 0) config.gatewayPort = port
  }
